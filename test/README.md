...
- [unit-disabled] Verifies that no resources are created when the module is disabled.
- [unit-minimal] Tests all defaults for optional arguments as well as required arguments.
...
2. Use Terratest to execute your _real_ IaC tools (e.g., Terraform, Packer, etc.)
   to deploy _real_ infrastructure (e.g., servers) in a _real_ environment (e.g., AWS, GCP).
...
**Note #1**: These tests create real resources in a cloud environment, which
means they cost money to run, especially if you don't clean up after
...
- For GCP set `GOOGLE_CREDENTIALS`.
- For GitHub set `GITHUB_TOKEN` and `GITHUB_OWNER`.
...
2. Install [Terraform].
3. Configure the used provider as written in the previous section.
...
[testdirectory]: https://github.com/mineiros-io/terraform-google-cloud-function/tree/main/test
[unit-disabled]: https://github.com/mineiros-io/terraform-google-cloud-function/blob/main/test/unit-disabled/main.tf
[unit-minimal]: https://github.com/mineiros-io/terraform-google-cloud-function/blob/main/test/unit-minimal/main.tf
[unit-complete]: https://github.com/mineiros-io/terraform-google-cloud-function/blob/main/test/unit-complete/main.tf
